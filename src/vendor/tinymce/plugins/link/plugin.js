...
define(
  ...
  function (Delay, Tools, XHR, Utils, Settings) {
    ...
    var createLinkList = function (editor, callback) {
      var linkListWrapper = Settings.getLinkList(editor.settings);
      var linkList = linkListWrapper.types;

      if (typeof linkList == "string") {
        XHR.send({
          url: linkList,
          success: function (text) {
            callback(editor, JSON.parse(text), linkListWrapper.pages);
          }
        });
      } else if (typeof linkList == "function") {
        linkList(function (list) {
          callback(editor, list, linkListWrapper.pages);
        });
      } else {
        callback(editor, linkList, linkListWrapper.pages);
      }
    };
    ...
    var showDialog = function (editor, linkList, pageCallback) {
      var data = {}, selection = editor.selection, dom = editor.dom, anchorElm, initialText;
      var win, onlyText, textListCtrl, linkListCtrl, secLinkListCtrl, relListCtrl, targetListCtrl, classListCtrl, linkTitleCtrl, value;

      var linkListChangeHandler = function (e) {
        var textCtrl = win.find('#text');

        if (!textCtrl.value() || (e.lastControl && textCtrl.value() == e.lastControl.text())) {
		  textCtrl.value(e.control.text().replace(/^\s+/, ''));
        }

        win.find('#href').value(e.control.value());
      };

		var typeLinkListChangeHandler = function (e) {
			pageCallback(e.control.value(), PagesListUpdateCallbak);
		};

		var PagesListUpdateCallbak = function(pages) {
			// Black Voodoo Magic
			if(secLinkListCtrl.menu){
				secLinkListCtrl.menu.remove();
				secLinkListCtrl.menu = null;
			}
			secLinkListCtrl.state.data.menu = secLinkListCtrl.settings.values = pages;
		};
      ...
      if (linkList) {
        linkListCtrl = {
          ...
          onselect: typeLinkListChangeHandler,
