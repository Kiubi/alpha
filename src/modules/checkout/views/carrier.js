...
var ScheduleView = Marionette.View.extend({
	...
	templateContext: function() {
		...
		var open_days = this.model.get('open_days');

		_.map(open_days, function(day) {
			day.name = moment().day(day.day_of_week).format('dddd');
			day.name = day.name.charAt(0).toUpperCase() + day.name.slice(1); // ucfirst

			day.time_frames = day.time_frames && _.isArray(day.time_frames) ? day.time_frames.join(',') : '';
		});
	...
	onSave: function() {
		...
		if (data.open_days) {
			_.map(data.open_days, function(day) {
				day.time_frames = day.time_frames == '' ? [] : day.time_frames.split(',');
			});
		}
